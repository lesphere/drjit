...
struct DiffVCall : CustomOp<DiffType, Result, ConstStr, Self, Func, Args...> {
    ...
    template <size_t... Is>
    void backward_impl(bool maintain_grad_array, std::index_sequence<Is...>) {
        const Self &self = Base::template value_in<1>();
        const Func &func = Base::template value_in<2>();
        ...
        auto func_bwd = [func](auto *self2, auto &grad_out,
            ...
            fprintf(
                stderr,
                "In vall_autodiff.h DiffVCall::backward_impl(maintain_grad_array = "
                "%d)\n",
                (int) maintain_grad_array);
            enqueue(ADMode::Backward, result);
            traverse<DiffType>(ADMode::Backward, (uint32_t) ADFlag::Default,
                               maintain_grad_array);
        ...
        Input grad_in = vcall_jit_record<Input>(
            name.get(), func_bwd, self, Base::grad_out(), maintain_grad_array,
    ...
    void backward(bool maintain_grad_array = false, py::object = py::none(), py::object = py::none(), int = 0,
                  py::object = py::none()) override {
        fprintf(stderr,
                "In vall_autodiff.h DiffVCall::backward(maintain_grad_array = "
                "%d)\n",
                (int) maintain_grad_array);
        backward_impl(maintain_grad_array,
