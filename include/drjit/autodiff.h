...
void ad_traverse(ADMode mode, uint32_t flags, bool maintain_grad_array = false, py::object opt = py::none(),
                 py::object guiding_t = py::none(), int id = 0, py::object model = py::none());
...
struct DiffArray : ArrayBase<value_t<Type_>, is_mask_v<Type_>, DiffArray<Type_>> {
    ...
    static void traverse_(ADMode mode, uint32_t flags,
        ...
        DRJIT_MARK_USED(flags);
        if constexpr (IsEnabled) 
            detail::ad_traverse<Type>(mode, flags, maintain_grad_array, opt, guiding_t, id, model);
    }
...
#define DRJIT_DECLARE_EXTERN_TEMPLATE(T, Mask, Index)                          \
    ...
    extern template DRJIT_AD_EXPORT void ad_traverse<T>(ADMode, uint32_t,      \
        bool, py::object, py::object, int, py::object);                        \
